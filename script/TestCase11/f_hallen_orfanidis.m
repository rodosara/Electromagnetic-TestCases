...
function [I,z,cnd] = f_hallen_orfanidis(L,a,M,Nint,type)

if nargin==0, help hallen; return; end
if nargin<=4, type=0; end
if nargin==3, Nint=1; end

eta = f_etac(1);                              % eta = 376.7303, approximately eta=120*pi
k = 2*pi;                                   % k = 2*pi/lambda, (lambda=1 units)
V0 = 1;                                     % assumed gap voltage V0 = 1

h = L/2;                                    % antenna half-length
Dz = h/(M + type*0.5);                      % sample spacing
G0 = Dz * (1j*eta/2/pi);                     % scaling factor

[w,x] = f_quadr(-1/2,1/2,Nint);               % Gauss-Legendre quadrature weights and points
                                            % Nint=1 has w = [1], x = [0]
m = (0:2*M)';    
